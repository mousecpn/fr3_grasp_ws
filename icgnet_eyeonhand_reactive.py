...
from grasp_planner.icgnet import ICGNetPlanner
from icg_net import ICGNetModule
from utils_exp.timer import Timer
T_base_wall = Transform(Rotation.identity(), [1, 0, 0])
round_id = 0

def get_model(config, device, graps_ori=6):
    return ICGNetModule(
        config=config,
        device=device,
...
class PandaGraspController(BasePandaGraspController):
    def __init__(self, vn=True):
        ...
        # /home/pinhao/icg_benchmark/data/icgnet/51--0.656/
        model = get_model("config.yaml","cuda")
        self.grasp_planner = ICGNetPlanner(
            model,
            confidence_th=0.1,
            resample=False,
            visualize=False,
    ...
    def run(self):
        ...
        poses = self.grasp_planner(pc,self.timer)
        if poses is None:
            return
        if len(poses) > 1:
        ...
        for i in range(len(poses)):
            ...
            # translation = translation - pose[:3,:3] @ np.array([0, 0, -0.032])

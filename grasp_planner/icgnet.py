...
class ICGNetPlanner(GraspPlannerModule[o3d.geometry.PointCloud]):
    ...
    def forward(self, pc, timer: Timer):
        if not self.latent_imagination:
            self.reset()
        ...
        with timer["postprocess"]:
            ...
            pose = torch.eye(4, device=self.device).unsqueeze(0).repeat(ori.shape[0],4,4)
            pose[:, :3, :3] = ori.reshape(-1, 3, 3)
            pose[:, :3, 3] = ctc[sort_idx] # - pose[:, :3, :3] @ torch.tensor([0, 0, 0.022]).to(self.device)
            return (pose.cpu().numpy(), score.reshape(-1,).cpu().numpy(), None)


            pose = torch.eye(4, device=self.device).unsqueeze(0)
            pose[:, :3, :3] = ori.view(3, 3)
            # Convert to edge grasp gripper convention
            pose[:, :3, 3] = ctc[best_grasps] - pose[:, :3, :3] @ torch.tensor([0, 0, 0.022]).to(self.device)
            return (pose.cpu().numpy(), score[best_grasps].reshape(-1,).cpu().numpy(), None)
